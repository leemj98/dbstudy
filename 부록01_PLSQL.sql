...
/*
    변수 선언하기
    1. 값을 저장할 때 대입 연산자를(:=)를 사용
    2. 타입을 선언하는 방식
        1) 스칼라 변수 : 타입을 직접 지정
        2) 참조 변수 : 특정 칼럼의 타입을 그대로 사용
*/

/*
    서버 메시지 출력하기
    1. 기본적으로 서버 메시지는 출력되지 않음
    2. 서버 메시지 출력을 위해서 최초 1회 아래 쿼리문을 실행
        SET SERVEROUTPUT ON;
    3. 출력하는 방법
        DBMS_OUTPUT.PUT_LINE(출력할 내용);
*/

-- 서버 메시지 출력 ON (디폴트값은 OFF)
SET SERVEROUTPUT ON; 
...
--2. 참조 변수(특정 칼럼의 타입을 명시하는 방법) **
DECLARE
    EMPLOYEEID EMPLOYEES.EMPLOYEE_ID%TYPE; --변수명 테이블.칼럼%TYPE (변수가 칼럼이랑 동일한 타입이라는 뜻)
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
    LASTNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
      INTO EMPLOYEEID, FIRSTNAME, LASTNAME --EMPLOYEE_ID는 EMPLOYEEID 변수에 넣어달라 이런뜻
      FROM EMPLOYEES
...
-- FOR문이 더 쉬운데 DB데이터를 가져와야하는데 그 숫자를 모르는 상태에서 가져올 때 등 WHILE문 써야할 때도 있음

/*
    보조 제어문
    EIXT : 반복문 (WHILE, FOR) 종료하기
    CONTINUE : 반복문의 시작부터 다시 실행하기 (무언가를 제외하고 싶을 때 씀)
*/
...
-- 1부터 누적. 3의 배수는 제외하고 누적. 누적 결과가 100이 넘어가면 그만 누적하고 결과 출력
DECLARE 
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER;
BEGIN
    ...
    WHILE TRUE LOOP
        -- N의 증가
        N := N+1; --이래야 CONTINUE로 올라온 숫자도 증가시켜줄 수 있음 (아니면 무한루프 빠지니까)
...
DECLARE
    ...
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
BEGIN
    EMPLOYEEID := 0; --EMPLOYEE_ID 값이 0인건 없어서 오류 발생
