...
-- 사원 테이블과 부서 테이블에 존재하는 모든 부서번호 조회
...
-- 활용2. WITH문과 재귀 쿼리를 활용한 상둰의 레벨 표시
-- MANAGER가 몇 명인가에 따른 LVL 표기하기
-- MANAGER가 0명 : LVL = 1
-- MANAGER가 1명 : LVL = 2
...
-- 초기값(LVL = 1)을 지정하는 서브쿼리
    SELECT 1, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID
     ...
     UNION ALL -- 똑같은 사람이 여러명 아닐테니 UNION도 상관없음
    -- 반복해서 호출되는 서브쿼리
