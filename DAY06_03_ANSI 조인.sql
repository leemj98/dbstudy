...
/*
    ...
    1. 드라이브 테이블
        ...
        3) 일반적으로 데이터의 갯수가 적음
        4) PK를 조인조건으로 사용하기 때문에 인덱스 사용이 가능 (빠르다는 의미)
    2. 드리븐 테이블
        1) 1:M 관계에서 M에 해당하는 테이블
        2) 일반적으로 데이터의 갯수가 많음
        3) FK를 조인조건으로 사용하기 때문에 인덱스 사용이 불가능 (느리다는 의미)    
    3. 드라이브 테이블을 드리븐 테이블보다 먼저 작성하면 성능에 도움이 됨
*/
...
SELECT EMPLOYEE_ID
     , FIRST_NAME
     , LAST_NAME
     , D.DEPARTMENT_ID --2개의 테이블에 이름이 같은 칼럼은 반드시 테이블(오너)을 명시해야 함
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
    -- 같은 이름으로 존재하는 칼럼은 반드시 오너 명시
...
SELECT EMPLOYEE_ID
     ...
     , D.DEPARTMENT_ID
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D RIGHT OUTER JOIN EMPLOYEES E -- 오른쪽 테이블(EMPLOYEES)의 모든 데이터를 조회하시오 (EMPLOYEES에서 부서번호가 없는 사원도 조회)
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
...
SELECT EMPLOYEE_ID
     ...
     , D.DEPARTMENT_ID
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E -- 왼쪽 테이블(DEPARTMENTS)의 모든 데이터를 조회하시오 (사원이 근무하지 않는 부서도 조회)
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;    
