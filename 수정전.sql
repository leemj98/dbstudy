...
CREATE TABLE ENROLL_TBL (
  ...
  , C_NO   VARCHAR2(5 BYTE) NOT NULL  -- COURSE_TBL  테이블의 C_NO 칼럼을 참조하는 외래키
  , E_DATE DATE   NULL
...
CREATE TABLE LECTURE_TBL (
  ...
  , E_NO       NUMBER             NULL  -- ENROLL_TBL 테이블의 E_NO 칼럼을 참조하는 외래키
  ...
  , CONSTRAINT FK_ENROLL_LEC FOREIGN KEY(E_NO) REFERENCES ENROLL_TBL(E_NO)    ON DELETE SET NULL
);
...
-- 4. 수강신청번호 시퀀스 + ENROLL_TBL 테이블 데이터 입력
...
-- 5. 강의번호 시퀀스 + LECTURE_TBL 테이블 데이터 입력
