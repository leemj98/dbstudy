...
SELECT O.ORDER_ID AS 주문번호, B.BOOK_ID AS 책번호, B.BOOK_NAME AS 책이름 
 ...
 WHERE TO_NUMBER(SUBSTR(O.ORDER_ID,1,6)) BETWEEN 200705 AND 200709
 ORDER BY O.ORDER_ID;
...
SELECT CUST_NAME AS 고객명
  FROM CUSTOMER_T
 WHERE CUST_ID NOT IN (SELECT CUST_ID FROM ORDER_T);
--조인으로 풀기
...
SELECT O.ORDER_ID AS 구매번호, C.CUST_NAME AS 구매자, B.BOOK_NAME AS 책이름, B.PRICE *O.AMOUNT AS 총구매액, O.ORDER_DT AS 주문일자
...
SELECT C.CUST_NAME AS 고객명, B.BOOK_NAME AS 책이름, O.ORDER_DT AS 주문일자
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON B.BOOK_ID = O.BOOK_ID
...
-- 김연아가 구매한 책이 몇 종인가
SELECT C.CUST_NAME AS 고객명, COUNT(O.ORDER_ID) AS 구매도서수, COUNT(DISTINCT O.BOOK_ID) AS 구매종류 --DISTINCT 같은 책번호는 중복제거
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID 
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;
...
-- 고객명  출판사수
-- 박지성  3
SELECT C.CUST_NAME AS 고객명, COUNT(B.PUBLISHER) AS 출판사수
  FROM BOOK_T B INNER JOIN ORDER_T O
